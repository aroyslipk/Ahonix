...
export function KpiCard({ kpi, currency, onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`group relative overflow-hidden rounded-xl border border-[#1E2235] bg-[#121420] p-5 card-hover ${
    ...
    >
        ...
        <div className="flex items-center gap-1.5 text-xs font-medium text-[#94A3B8]">
          ...
          <TooltipProvider delayDuration={120}>
            <Tooltip>
              ...
              <TooltipContent className="max-w-[220px] border-[#2D334B] bg-[#0B0D14] text-xs text-[#CBD5E1]">
                {kpi.tooltip}
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
      ...
      <div className="flex items-end justify-between gap-2">
        <div>
          <p className="font-metric text-2xl font-extrabold text-[#F8FAFC] sm:text-3xl">
          ...
          <div className="mt-2 flex items-center gap-2 text-xs">
            {kpi.id === "marketing-eff" && (!kpi.value || kpi.value === 0) ? (
              ...
              <>
                <span className={`inline-flex items-center rounded px-1.5 py-0.5 font-semibold ${changeColor} ${changeBg}`}>
