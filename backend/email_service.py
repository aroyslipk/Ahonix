...
async def send_password_reset_email(to_email: str, reset_token: str, frontend_url: Optional[str] = None) -> bool:
    """Dispatch password reset email using configured transport (SMTP or safe dev fallback).
    ...
    if cfg["smtp_host"]:
        ...
        try:
            success = await asyncio.to_thread(_send_smtp_sync, to_email, subject, html_content, text_content, cfg)
            return success
        except Exception as e:
        ...
        # Log delivery notice without exposing the secret token or reset URL
        logger.info(
            "Email service in development mode (SMTP_HOST not set). Simulated reset email delivery to %s. (Token omitted for security)",
            mask_email(to_email),
        )
        return True
